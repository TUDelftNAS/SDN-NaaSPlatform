...
# SNMP based interface bandwidth utilization monitoring application - Face - a MPLS (i.e. Juniper Legacy Switch) core switch
...
while True:
    ...
    with open("Logs/Face_If_In_Octet.json") as json_file:
            if_in_octet_prev_stat = {}
            if_in_octet_prev_stat = json.load(json_file)
    with open("Logs/Face_If_Out_Octet.json") as json_file:
    ...
    for (ifis, ifos) in zip(if_in_stat, if_out_stat) :
        ...
        if (obj_list[10] == '509' or obj_list[10] == '503' or obj_list[10] == '505'):
            ...
            print '\n\n'
    with open("Events/Interface/Face_Utilizations.json", "w") as json_file:
        json.dump(high_uti_events, json_file)
    with open("Logs/Face_If_In_Octet.json", "w") as json_file:
        json.dump(if_in_octet_stat, json_file)
    with open("Logs/Face_If_Out_Octet.json", "w") as json_file:
