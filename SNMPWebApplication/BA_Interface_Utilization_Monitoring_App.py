...
# SNMP based interface bandwidth utilization monitoring application - BA - a MPLS (i.e. Juniper Legacy Switch) core switch

print '\n\n\n***Welcome to the SNMP based Interface Bandwidth Utilization Monitoring Application'
print 'For SNMP Agent - BA - a MPLS (i.e. Juniper Legacy Switch) Core Switch***\n\n'
...
# SNMP Agent Configuration Details (i.e. BA)

agent_ip='139.63.246.115'
agent_port = 161
community='public'
...
comm_data = cmdgen.CommunityData('BA', community, 1) 
...
with open("Config/Device_Interfaces/BA_Interface_List.json") as json_file:
            if_list = {}
...
while True:
    ...
    with open("Logs/BA_If_In_Octet.json") as json_file:
            if_in_octet_prev_stat = {}
            if_in_octet_prev_stat = json.load(json_file)
    with open("Logs/BA_If_Out_Octet.json") as json_file:
    ...
    for (ifis, ifos) in zip(if_in_stat, if_out_stat) :
        ...
        obj = str(obj)
        obj_list = obj.split(".")
        if (obj_list[10] == '504' or obj_list[10] == '508' or obj_list[10] == '510' or obj_list[10] == '516'):
            ...
            print '\n\n'
    with open("Events/Interface/BA_Utilizations.json", "w") as json_file:
        json.dump(high_uti_events, json_file)
    with open("Logs/BA_If_In_Octet.json", "w") as json_file:
        json.dump(if_in_octet_stat, json_file)
    with open("Logs/BA_If_Out_Octet.json", "w") as json_file:
