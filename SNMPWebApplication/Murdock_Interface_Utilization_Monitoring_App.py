...
# SNMP based interface bandwidth utilization monitoring application - Murdock - a MPLS (i.e. Juniper Legacy Switch) core switch

print '\n\n\n***Welcome to the SNMP based Interface Bandwidth Utilization Monitoring Application'
print 'For SNMP Agent - Murdock - a MPLS (i.e. Juniper Legacy Switch) Core Switch***\n\n'
...
while True:
    ...
    with open("Logs/Murdock_If_In_Octet.json") as json_file:
            if_in_octet_prev_stat = {}
            if_in_octet_prev_stat = json.load(json_file)
    with open("Logs/Murdock_If_Out_Octet.json") as json_file:
    ...
    for (ifis, ifos) in zip(if_in_stat, if_out_stat) :
        ...
        obj = str(obj)
        obj_list = obj.split(".")
        if (obj_list[10] == '504' or obj_list[10] == '509' or obj_list[10] == '513' or obj_list[10] == '508'):
            ...
            print '\n\n'
    with open("Events/Interface/Murdock_Utilizations.json", "w") as json_file:
        json.dump(high_uti_events, json_file)
    with open("Logs/Murdock_If_In_Octet.json", "w") as json_file:
        json.dump(if_in_octet_stat, json_file)
    with open("Logs/Murdock_If_Out_Octet.json", "w") as json_file:
